...
def make_investor_profile_medical(path: Path) -> None:
    meta = {
        "title": "康桥产业基金医疗器械投资画像",
        "subject": "Medical investor-side private-file retrieval fixture",
        "role": "投资人侧私有资料",
        "code": "INV-MEDI-083",
    }
    story = document_intro(
        kicker="INVESTOR MANDATE｜MEDTECH｜2026 Q3",
        title="康桥产业基金｜医疗器械投资画像",
        subtitle="用于验证医疗健康成长期投资人侧文件检索、注册临床偏好识别与双方文件隔离。",
        marker="INV-MEDI-083",
    ...
    story += [
        ...
        identity_table(
            [
                ("Agent 名称", "康桥产业基金·顾宁（虚构代号）"),
                ("机构类型", "产业资本 / CVC，人民币成长期基金"),
                ("决策角色", "产业投资负责人；可立项并推进投委会，最终决策需投委会"),
                ("当前节奏", "选择性出手；重点看注册、临床与医院商业化节点"),
        ...
        data_table(
            ["维度", "偏好", "补充说明"],
            [
                ["核心赛道", "体外诊断、高值耗材、医疗设备", "优先中国大陆三类医疗器械与医院场景；不投无注册路径的“概念设备”。"],
                ["阶段", "B 轮、C 轮", "要求已有注册证或明确注册路径，且存在持续采购或装机验证。"],
                ["首次支票", "5000 万-1 亿元人民币", "可领投；典型目标持股 10%-20%。"],
                ["地域", "中国大陆为主", "若涉及进口替代或海外注册，需尽早说明 NMPA 与海外路径。"],
            ],
            [30 * mm, 49 * mm, 86 * mm],
        ),
        heading("03｜最影响初筛的信号"),
        *bullets(
            [
                "注册与临床节点：是否已取得或接近关键注册证、临床验证或准入证据。",
                "医院商业化质量：区分装机、试点、正式采购与耗材复购；不得把装机等同于收入。",
                "供应链与产能：关注关键物料、产线爬坡和批次一致性。",
                "合规路径：牌照、数据授权、IP 归属必须在正式尽调前可说明。",
            ]
        ),
        callout(
            "<b>最小证据要求：</b>收入与装机数据需给出统计口径与截止日期；可匿名医院，无需初筛阶段提供完整合同。",
            "gold",
        ),
        PageBreak(),
        ...
        data_table(
            ["类型", "内容", "处理方式"],
            [
                ["明确排除", "无注册路径的高风险器械、数据无法说明来源、纯概念设备", "初筛即可建议结束，但必须说明触发条件。"],
                ["条件性", "二类/三类器械、医院数据跨境、竞品关联", "先要求说明注册进度、数据权利与利益冲突隔离。"],
                ["利益冲突", "与已投体外诊断或高值耗材项目直接竞争", "在阅读详细商业机密前确认信息隔离范围。"],
            ],
            [30 * mm, 82 * mm, 53 * mm],
        ),
        ...
        *bullets(
            [
                "医院渠道与产业方引荐：在立项后根据场景适配度协调。",
                "注册临床、供应链与医院商业化节奏讨论。",
                "后续轮融资材料反馈与机构介绍；不保证融资结果。",
            ]
        ),
        heading("06｜检索验收提示"),
        callout(
            "可用问题：“康桥产业基金的典型首次支票是多少？”“它最看重哪些注册或临床节点？”“为什么某项目只能进入条件性复核？”",
            "teal",
        ),
    ]
...
def make_founder_pitch_medical(path: Path) -> None:
    meta = {
        "title": "循准医疗 B 轮融资简报",
        "subject": "Medical founder-side pitch retrieval fixture",
        "role": "创业者侧私有资料",
        "code": "FOUNDER-PULSE-562",
    }
    story = document_intro(
        kicker="FOUNDER BRIEF｜B 轮｜2026-06",
        title="循准医疗｜B 轮融资简报",
        subtitle="面向三级医院检验科的多重病原快检平台，通过微流控耗材与配套仪器缩短检测时间并提供标准化质控。",
        marker="FOUNDER-PULSE-562",
    ...
    story += [
        ...
        identity_table(
            [
                ("公司与代号", "循准医疗（虚构项目） / 创始人代号“许澄”"),
                ("目标客户", "三级医院检验科，首先覆盖呼吸道与多重感染筛查场景"),
                ("产品阶段", "已取得注册证并进入医院规模化采购；耗材持续复购验证中"),
                ("融资轮次", "B 轮，计划融资 5000 万-1 亿元人民币"),
        ...
        data_table(
            ["项目", "已确认内容", "尚未证明"],
            [
                ["客户问题", "多重感染筛查耗时长、质控分散、结果追溯困难。", "不同科室与医院等级的需求是否足够标准化。"],
                ["产品", "微流控耗材 + 配套仪器 + 标准化质控流程。", "在更复杂样本类型上的扩展成本。"],
                ["安全边界", "产品提供辅助检测，不自动出具临床结论，不取代检验科负责人审批。", "还需按客户信息安全等级完成更多第三方测试。"],
            ],
            [31 * mm, 76 * mm, 58 * mm],
        ),
        heading("03｜团队"),
        *bullets(
            [
                "CEO：覆盖体外诊断商业化与医院渠道，负责商业与组织。",
                "CTO：覆盖微流控与仪器平台，负责产品架构。",
                "注册临床负责人：来自三甲医院检验体系，负责注册与临床路径。",
                "当前全职 46 人；核心成员均为全职。此处不包含身份证、住址或个人联系方式。",
            ]
        ),
        PageBreak(),
        heading("04｜经营指标（截至 2026-06-30）"),
        callout(
            "以下均为合成数据。“医院数”指已签有偿合同或持续采购的医疗机构；“耗材复购”只计算在观察期内已到期的客户。",
            "gold",
        ),
        Spacer(1, 6),
        data_table(
            ["指标", "当前值", "口径 / 限制"],
            [
                ["进入医院", "42 家", "其中 28 家持续采购耗材。"],
                ["近 12 个月确认收入", "6800 万元", "包含仪器与耗材；不等同于会计收入全部口径。"],
                ["仪器装机", "63 台", "含试用转正式装机；不应直接等同于已签收入。"],
                ["耗材毛利率", "68%", "未计入总部研发与销售费用。"],
                ["到期客户续约", "5 / 6", "样本小，不应直接外推为长期续费率。"],
                ["中位回款周期", "95 天", "受医院采购与财政付款节奏影响。"],
            ],
            [41 * mm, 38 * mm, 86 * mm],
        ),
        heading("05｜融资用途与里程碑"),
        data_table(
            ["用途", "占比", "12-18 个月目标"],
            [
                ["产线与供应链", "35%", "新增产线爬坡，关键物料国产化率提升。"],
                ["注册与临床", "20%", "完成两个新检测面板注册与临床验证。"],
                ["市场与销售", "25%", "聚焦 3 个重点省份医院渠道，不盲目扩张装机量。"],
                ["客户成功与交付", "20%", "建立可复用医院上线模板，缩短装机到验收周期。"],
            ],
            [42 * mm, 25 * mm, 98 * mm],
        ),
        ...
        *bullets(
            [
                "医院回款周期带来阶段性现金流压力，新增产线仍在爬坡。",
                "竞品关联与产业方利益冲突需提前披露并隔离。",
                "希望获得医院商业化验证、供应链优化和后续融资材料反馈。",
            ]
        ),
        heading("07｜检索验收提示"),
        callout(
            "可用问题：“近 12 个月确认收入是否包含装机？”“持续采购耗材的医院有多少家？”“本轮融资如何分配？”",
            "teal",
        ),
    ]
    build_pdf(path, meta, story)
...
def make_financial_appendix_medical(path: Path) -> None:
    meta = {
        "title": "循准医疗经营指标附录",
        "subject": "Medical confidential metric-definition retrieval fixture",
        "role": "创业者侧高敏资料",
        "code": "FIN-CLIN-416",
    }
    story = document_intro(
        kicker="CONFIDENTIAL METRICS APPENDIX｜MEDTECH｜SYNTHETIC",
        title="循准医疗｜经营指标与单位经济模型附录",
        subtitle="用于测试精确数值检索、医院采购口径理解、多页命中与私有文件隔离。",
        marker="FIN-CLIN-416",
        classification="创业者侧高敏 / 需最小化引用",
    )
    story += [
        callout(
            "<b>使用边界：</b>所有医院均以代码表示，数值为合成测试数据。初筛时只引用回答具体问题所必需的片段，不应主动向对方倾倒全表。",
            "red",
        ),
        heading("01｜指标定义"),
        data_table(
            ["名称", "本文档定义", "常见误读"],
            [
                ["确认收入", "截止日已确认仪器与耗材收入；不含未验收装机。", "不能与合同签约额或装机台数混用。"],
                ["耗材复购率", "只对同一期初已经存在且期末仍在观察窗口的耗材客户计算。", "当前样本小，不能把六个客户的结果当作稳定趋势。"],
                ["耗材毛利", "耗材收入减去物料、制造、质控与客户支持直接成本。", "未扣除研发、销售与管理费用，不是净利率。"],
                ["装机转化周期", "从医院试用到正式采购或持续耗材采购的中位天数。", "单季度波动很大，只能作方向性参考。"],
            ],
            [31 * mm, 77 * mm, 57 * mm],
        ),
        heading("02｜月度走势"),
        data_table(
            ["月份", "确认收入", "耗材收入", "期末现金", "全职人数"],
            [
                ["2026-01", "480 万元", "210 万元", "4200 万元", "41"],
                ["2026-02", "510 万元", "225 万元", "3980 万元", "42"],
                ["2026-03", "540 万元", "240 万元", "3710 万元", "43"],
                ["2026-04", "560 万元", "255 万元", "3460 万元", "44"],
                ["2026-05", "590 万元", "270 万元", "3180 万元", "45"],
                ["2026-06", "620 万元", "285 万元", "2890 万元", "46"],
            ],
            [31 * mm, 41 * mm, 35 * mm, 34 * mm, 24 * mm],
        ),
        callout(
            "<b>计算验收：</b>2026-06 确认收入为 620 万元；近 12 个月合计约 6800 万元。装机台数 63 台不计入确认收入。",
            "teal",
        ),
        PageBreak(),
        heading("03｜医院级别样本（匿名）"),
        data_table(
            ["医院代码", "科室", "当前形态", "年度耗材", "关键状态"],
            [
                ["H-ALPHA", "检验科", "持续采购", "420 万元", "已续约一次；两院区使用。"],
                ["H-BRAVO", "检验科", "持续采购", "360 万元", "扩容尚在采购流程。"],
                ["H-CHARLIE", "检验科", "正式装机", "不适用", "装机费 180 万元；不应计入耗材复购。"],
                ["P-ECHO", "检验科", "试用", "不适用", "试用装机 6 台；不应计入确认收入。"],
            ],
        ...
        data_table(
            ...
            [
                ["新医院 CAC 中位数", "86 万元", "只有 5 个新客户样本，且销售费用跨期，可信度低。"],
                ["耗材毛利率", "68%", "制造与质控已计入；研发与销售未计入。"],
                ["粗略回收期", "约 22 个月", "基于中位合同值和当前毛利；尚未考虑扩容与流失。"],
                ["现金跑道", "约 8-9 个月", "以 2026-06 期末现金和近三月平均净消耗估算；不含未签融资。"],
            ],
            [44 * mm, 39 * mm, 82 * mm],
        ),
        heading("05｜检索验收提示"),
        callout(
            "可用问题：“2026-06 的确认收入是多少？”“H-BRAVO 的扩容是已签收入吗？”“63 台装机是否等于确认收入？”",
            "teal",
        ),
    ]
    build_pdf(path, meta, story)
...
def generate_all(output_dir: Path) -> list[Path]:
    ...
    outputs = [
        ...
        output_dir / "investor_profile_medical_cn.pdf",
        output_dir / "founder_pitch_medical_cn.pdf",
        output_dir / "founder_financial_appendix_medical_cn.pdf",
    ...
    makers = [
        ...
        make_investor_profile_medical,
        make_founder_pitch_medical,
        make_financial_appendix_medical,
