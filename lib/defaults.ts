...
export const MEMORY_PROMPTS: Record<AgentRole, string> = {
  investor: `你负责从本轮对话中提取投资人 Agent 关于创业者的增量长期记忆。当前工作状态仅作去重参考；必须依据【完整对话】提取本轮新出现、且未来仍有用的信息。不要重写整份记忆，不要重复已有条目，不要把寒暄、临时措辞或无关细节写入长期记忆。即使工作状态里存在“旧版整块私有记忆”占位，也不能据此输出空数组——只要本轮对话出现了对方新事实、偏好、约束或值得保留的判断，就必须写入。
只输出合法 JSON：{"memories":[{"kind":"fact","title":"","content":"","verification":"unverified","priority":50,"source_turns":[]}]}
kind 只能是 fact、preference、constraint、note；对方自报内容一律使用 unverified，只有双方资料中已明确保存的本方事实才可用 confirmed。priority 为 0–100。Agent 自己提出的建议不得写成用户已确认 decision；用户决策只能由用户直聊确认后写入。新旧信息冲突时使用 verification=conflicted，并在内容中同时保留两种说法，不得擅自选边。最多输出 12 条，仅当本轮对话确实没有任何新信息时才输出空数组。`,
  founder: `你负责从本轮对话中提取创业者 Agent 关于投资人的增量长期记忆。当前工作状态仅作去重参考；必须依据【完整对话】提取本轮新出现、且未来仍有用的信息。不要重写整份记忆，不要重复已有条目，不要把寒暄、临时措辞或无关细节写入长期记忆。即使工作状态里存在“旧版整块私有记忆”占位，也不能据此输出空数组——只要本轮对话出现了对方新事实、偏好、约束或值得保留的判断，就必须写入。
只输出合法 JSON：{"memories":[{"kind":"fact","title":"","content":"","verification":"unverified","priority":50,"source_turns":[]}]}
kind 只能是 fact、preference、constraint、note；对方自报内容一律使用 unverified，只有双方资料中已明确保存的本方事实才可用 confirmed。priority 为 0–100。Agent 自己提出的建议不得写成用户已确认 decision；用户决策只能由用户直聊确认后写入。新旧信息冲突时使用 verification=conflicted，并在内容中同时保留两种说法，不得擅自选边。最多输出 12 条，仅当本轮对话确实没有任何新信息时才输出空数组。`,
};
